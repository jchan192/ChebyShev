...
void Assign_BC(vector_patch &Patch,
  ...
  for (int D=0;D<NsubD;D++) {
	...
	Patch[D].psi[ig] = GWP_vals(Patch[D].x[ig],(tn-1.)*par.dt);
	Patch[D].tmp[ig] = GWP_vals(Patch[D].x[ig],(tn-1.)*par.dt);
      }
    }
    ...
    if (D==(NsubD-1)){
      for (int ig=0; ig< ghost_pts; ig++){
	Patch[D].psi[N-ig] = GWP_vals(Patch[D].x[N-ig],(tn-1.)*par.dt);
	Patch[D].tmp[N-ig] = GWP_vals(Patch[D].x[N-ig],(tn-1.)*par.dt);
      }
    }
    //    else if (Patch[D+1].level==0){
    else {
