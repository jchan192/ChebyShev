...
complex GWP_vals(double x,double Time)
{
  ...
  const double Gau_v0  = 4.0;                    //mean velocity [1.0]
...
void Assign_BC(vector_patch &Patch,
  ...
  for (int D=0;D<NsubD;D++) {
    if (D==0){
      ...
      Patch[D].tmp[0] = Patch[D].a;
      // Patch[D].a = Patch[NsubD-1].tmp[Patch[NsubD-1].N-1]; 
      // Patch[D].psi[0] = Patch[D].a;
      // Patch[D].tmp[0] = Patch[D].a;
    }
    ...
    if (D==(NsubD-1)){
      ...
      Patch[D].tmp[Patch[D].N] = Patch[D].b;
      // Patch[D].b = Patch[0].tmp[1]; 
      // Patch[D].psi[Patch[D].N] = Patch[D].b;
      // Patch[D].tmp[Patch[D].N] = Patch[D].b;
    }
    else if (Patch[D+1].level==0){
