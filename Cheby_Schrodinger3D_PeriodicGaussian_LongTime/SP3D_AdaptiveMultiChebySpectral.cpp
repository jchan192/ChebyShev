...
complex GWP_vals(double x,double y,double z, double Time)
{
  ...
  double hbar = 1.;
  double m =1.;
  double  sigma_px = 10, sigma_py=10,sigma_pz=10;
...
void Print_XYZ(Params &par,
  ...
  filename_fdm << "output7/XYZ" << tn << ".dat";
...
void Error(Params &par,
  ...
  for (size_t k = 1; k < size-1; ++k){
    ...
    Error += pow((psi[k+size*(j+i*(size))].real() * psi[k+size*(j+i*(size))].real() 
		+ psi[k+size*(j+i*(size))].imag() * psi[k+size*(j+i*(size))].imag())
