...
vector_complex cheby_1Dcoeff(vector_complex y, double N)
{
...
vector_complex cheby_1Dinterp(vector_real &x, vector_complex &coeff,double N,double a,double b)
{
  int Nx = x.size();
...
void Refinement(Patches &P){
  ...
  // 	      << P.psi[i].imag() <<"\n";
  // }
  P.coeff = cheby_1Dcoeff(P.psi,P.N);
...
void Assign_BC(vector_patch &Patch,
  ...
  for (int D=0; D<NsubD*NsubD; D++)
    cheby_2Dcoeff(Patch[D].psi,Patch[D].N,Patch[D].coeff,Patch[D]);
  ...
  for (int Dj=0;Dj<NsubD;Dj++) {
	...
	Patch[D].tmp[ig*(N+1)+jg] =  GWP_vals(Patch[D].x[ig],Patch[D].y[jg],(tn-1.)*par.dt);
      ...
      for (int jg=ghost_pts; jg<N+1-ghost_pts; jg++){
      for (int ig=0; ig< ghost_pts; ig++){
	  Patch[D].psi[ig*(N+1)+jg] =  cheby_2Dinterp(Patch[D].x[ig],
						    Patch[D].y[jg],
						    Patch[Dfrom].coeff,
						    Patch[Dfrom].N,
						    Patch[Dfrom].x[0],
...
//     // right face
	...
	//	std::cout << "b"<<j<<"="<< Patch[D].b[j] <<"\n";
	  ...
	  Patch[D].psi[(N-ig)*(N+1)+jg] =  cheby_2Dinterp(Patch[D].x[N-ig],
							Patch[D].y[jg],
							Patch[Dfrom].coeff,
							Patch[Dfrom].N,
							Patch[Dfrom].x[0],
...
//     // D face
	...
	//	std::cout << "d"<<i<<"="<< Patch[D].d[i] <<"\n";
	  ...
	  Patch[D].psi[ig*(N+1)+N-jg] =  cheby_2Dinterp(Patch[D].x[ig],
						    Patch[D].y[N-jg],
						    Patch[Dfrom].coeff,
						    Patch[Dfrom].N,
						    Patch[Dfrom].x[0],
...
//     // upper-right corner
   if (Di==(NsubD-1) || (Dj==(NsubD-1))){
	...
	for (int ig=0; ig< ghost_pts; ig++){
	for (int jg=0; jg< ghost_pts; jg++){	    
	  Patch[D].psi[(N-ig)*(N+1)+N-jg] =  cheby_2Dinterp(Patch[D].x[N-ig],
							  Patch[D].y[N-jg],
							  Patch[Dfrom].coeff,
							  Patch[Dfrom].N,
							  Patch[Dfrom].x[0],
...
void Assign_1D2DBC(vector_patch &Patch,
	       signed long int NsubD,
	       Params &par,
	       double tn){
  ...
  int N , D, Dfrom;
	...
	// 	    <<  Patch[Dfrom].x[t] << " "
	// 	    <<  Patch[Dfrom].psi[t*(N+1)+jg].real() << " " 
	// 	    <<	cheby_1Dinterp(Patch[Dfrom].x[t],
	// 			       coeff1D,
	// 			       Patch[Dfrom].N,
	// 			       Patch[Dfrom].x[0],
	// 			       Patch[Dfrom].x.back()).real() << " " 
	// 	    <<  GWP_vals(Patch[Dfrom].x[t],
	// 			 Patch[Dfrom].y[jg],
	// 			 (tn-1.)*par.dt).real() << "\n";	  
	// std::exit(0);
// 	for (int t=0; t< 100; t++)
// 	  std::cout << std::setprecision(20)
// 		    <<  Patch[Dfrom].x[0]+t*(Patch[Dfrom].x.back()-Patch[Dfrom].x[0])/(99.) << " "
// //		    <<  psi1D[t].real() << " " 
// 		    <<	cheby_1Dinterp((double)(Patch[Dfrom].x[0]+(double)t*(Patch[Dfrom].x.back()-Patch[Dfrom].x[0])/99.),
// 				       coeff1D,
// 				       Patch[Dfrom].N,
// 				       Patch[Dfrom].x[0],
// 				       Patch[Dfrom].x.back()).real() << " " 
// 		    <<  GWP_vals((Patch[Dfrom].x[0]+(double)t*(Patch[Dfrom].x.back()-Patch[Dfrom].x[0])/99.),
// 				 Patch[Dfrom].y[jg],(tn-1.)*par.dt).real() << "\n";	  
...
// 	std::exit(0);
  ...
  for (int Dj=0;Dj<NsubD;Dj++) {
	...
	for (int t=0; t< N+1; t++)
	  psi1D[t] = Patch[Dfrom].psi[t*(N+1)+jg];

	coeff1D = cheby_1Dcoeff(psi1D, 
				Patch[Dfrom].N);

	for (int ig=0; ig< ghost_pts; ig++){
	  Patch[D].psi[ig*(N+1)+jg] = cheby_1Dinterp(Patch[D].x[ig],
						     coeff1D,
						     Patch[Dfrom].N,
						     Patch[Dfrom].x[0],
						     Patch[Dfrom].x.back());
...
//     // right face
	...
	coeff1D = cheby_1Dcoeff(psi1D, Patch[Dfrom].N);
	for (int ig=0; ig< ghost_pts; ig++){

	  Patch[D].psi[(N-ig)*(N+1)+jg] = cheby_1Dinterp(Patch[D].x[N-ig],
						 coeff1D,
						 Patch[Dfrom].N,
						 Patch[Dfrom].x[0],
						 Patch[Dfrom].x.back());
...
// //     // C face
	...
	for (int t=0; t< N+1; t++)
	  psi1D[t] = Patch[Dfrom].psi[ig*(N+1)+t];

	coeff1D = cheby_1Dcoeff(psi1D, Patch[Dfrom].N);

      for (int jg=0; jg< ghost_pts; jg++){

	  Patch[D].psi[ig*(N+1)+jg] = cheby_1Dinterp(Patch[D].y[jg],
						 coeff1D,
						 Patch[Dfrom].N,
						 Patch[Dfrom].y[0],
						 Patch[Dfrom].y.back());
...
// //     // D face
	...
	coeff1D = cheby_1Dcoeff(psi1D, Patch[Dfrom].N);
	  ...
	  Patch[D].psi[ig*(N+1)+N-jg] = cheby_1Dinterp(Patch[D].y[N-jg],
						 coeff1D,
						 Patch[Dfrom].N,
						 Patch[Dfrom].y[0],
						 Patch[Dfrom].y.back());
   ...
   if (Di==0 || (Dj==0)){
	...
	// 1D diagonal
// 	Ldiag = pow(pow(Patch[Dfrom].x.back()-Patch[Dfrom].x[0],2) +  pow(Patch[Dfrom].y.back()-Patch[Dfrom].y[0],2),0.5);
// //	std::cout << Ldiag <<" " << pow(Patch[Dfrom].x.back()-Patch[Dfrom].x[0],2) <<"\n";
// //	std::exit(0);

// 	for (int t=0; t< N+1; t++){
// 	  psi1D[t] = Patch[Dfrom].psi[t*(N+1)+t];
// 	}
// 	coeff1D = cheby_1Dcoeff(psi1D, 
// 				Patch[Dfrom].N);

// 	for (int ig=0; ig< ghost_pts; ig++){
// 	  Pdiag = Ldiag - pow(pow(Patch[Dfrom].x.back()-Patch[D].x[ig],2) +  pow(Patch[Dfrom].y.back()-Patch[D].y[ig],2),0.5);
// //	  std::cout << Pdiag <<" " << pow(Patch[Dfrom].x.back()-Patch[Dfrom].x[0],2) <<"\n";
// 	  Patch[D].psi[ig*(N+1)+ig] = cheby_1Dinterp(Pdiag,
// 						     coeff1D,
// 						     Patch[Dfrom].N,
// 						     0,
// 						     Ldiag);
...
void Assign_1D2DCornerBC(vector_patch &Patch,
	       signed long int NsubD,
	       Params &par,
	       double tn){
...
//  Assign_BC(Patch,NsubD,par,tn);
  Assign_1D2DBC(Patch,NsubD,par,tn);
   // for (int D=0; D<NsubD*NsubD; D++)
...
//    if ((int)tn == 1 ) std::exit(0);
  ...
  for (int D=0;D<NsubD*NsubD;D++) {
	...
	Patch[D].k1[i*(Patch[D].N+1)+j] += Patch[D].Dc[j*(Patch[D].N+1)+t] * Patch[D].psi[i*(Patch[D].N+1)+t] ;
...
//  Assign_1D2DCornerBC(Patch,NsubD,par,tn);
  ...
  for (int D=0;D<NsubD*NsubD;D++) {
	...
	Patch[D].k1[j*(Patch[D].N+1)+i] += Patch[D].Dc[j*(Patch[D].N+1)+t] * Patch[D].psi[t*(Patch[D].N+1)+i] ;
    }
    }}}
...
//    Print_Patch(par,Patch,NsubD,tn,0);
  ...
  std::cout << "Error =" << pow(err/pow(NN,2),1./2) << "\n";
  std::cout << NN <<"\n";
